...
def plot_function_map(f, nb_sub=10, ax=None, xlim=[-1, 1], ylim=[-1, 1], opp=False, exp=False, vmin=None, vmax=None,
                      contour=False, zorder=None):
    """
    ...
    if contour:
        try:
            CS = ax.contour(xx, yy, z, cmap='viridis', alpha=0.6)
...
def plot_mixture_linear_system(model, mode='glob', nb_sub=20, gmm=True, min_alpha=0.,
    ...
    if mode == 'glob':
        ...
        strm = plot_linear_system(None, model.center, field=field.T, nb_sub=nb_sub,
                                  multi_center=hasattr(model, 'unconstrained_center'),
                                  **kwargs)
