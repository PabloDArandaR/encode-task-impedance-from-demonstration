	...
	for ti in range(0,len(tim)-window_size): # loop to each window size
		...
		for i in range(10000): # learning for 10,000 iterations (in maximum)
			...
			for pi in range(len(parameters)):
				...
				if mom[pi] is None:
					mom[pi] =  + learning_rate*grd[pi]
				else:
					mom[pi] =  0.0*mom[pi] + learning_rate*grd[pi]
				parameters[pi] = parameters[pi] - mom[pi]
			#optimizer.step()
			...
			'''if ((previousloss-loss.item()) < loss_threshold) and (previousloss >= loss.item()) :
				break'''
			if ((loss.item() < loss_threshold) or ((previousloss-loss.item()) < 0.001*loss_threshold)) and (i > 10):
				break
